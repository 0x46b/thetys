...
init_configurations(sensor_configuration_data *configs, size_t initialSize) {
  ...
  ESP_LOGI(CONF_TAG, "Successfully initialized memory for %i configurations.",
           initialSize);
  return SUCCESS;
}
...
insert_configuration(sensor_configuration_data *configs,
  ...
  if (configs->configurations == NULL) {
    ESP_LOGE(CONF_TAG, "Try to insert into uninitialized array");
  ...
  if (configs->used == configs->size) {
    ...
    if (new_array == NULL) {
      ESP_LOGE(CONF_TAG, "Resizing failed");
      return LOW_MEMORY;
    }
    configs->size = new_size;
    configs->configurations = new_array;
...
free_configurations(sensor_configuration_data *configs) {
  ...
  configs->used = 0;
  configs->size = 0;
  ESP_LOGI(CONF_TAG, "Reset configurations");
  return SUCCESS;
}
