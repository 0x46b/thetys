...
typedef enum SMGR_RESULT {
  ...
  IO_ERROR = 1, /*!< The sensor is not readably due hardware-failure */
  UNKNOWN_SENSOR =
      2, /*!< The sensor with the given id is not registered for handling */
  MISSING_CALIBRATION = 3, /*!< Calibration-data for sensor is missing */
  CATASTROPHIC_FAILURE =
      4, /*!< Something went terribly wrong, so we could not recover from it */
  UNKNOWN_ERROR = 255 /*!< Something went wrong, but we dunno what */
} SMGR_RESULT;
 ...
 * @return -1 on error, the sensor_id otherwise
 */
 ...
 * water")
 * @param sensor_id The id of the sensor we want to calibrate
 * @return SMGR_RESULT::SUCCESS Calibration was successfull
 * @return SMGR_RESULT::UNKNOWN_SENSOR No sensor with sensor_id was found
 * @return SMGR_RESULT::IO_ERROR We could not read from the sensor hardware-wise
 */
SMGR_RESULT calibrate_water(uint32_t sensor_id);

/*! Reads the current value for the given sensor
 * @param sensor_id The id of the sensor we want to calibrate
 * @return SMGR_RESULT::SUCCESS Reading was successfull and data is stored into
 * reading
 * @return SMGR_RESULT::UNKNOWN_SENSOR No sensor with sensor_id was found (Note:
 * reading will not get initialized)
 * @return SMGR_RESULT::IO_ERROR We could not read from the sensor hardware-wise
 * (Note: reading will not get initialized)
 */
SMGR_RESULT get_sensor_data(uint32_t sensor_id, sensor_reading *reading);
#endif
